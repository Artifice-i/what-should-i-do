...
import {recommend as recommendV1} from './engine.js';
import {recommendSocal} from './socal-engine.js';
import {sources} from './socal-sources.js';
const recommend=(p,r)=>edition==='socal'?recommendSocal(p,r):recommendV1(p,r);
let edition='v1';
const form=document.querySelector('form'),result=document.querySelector('#result');
const sessions={v1:{rejected:new Set(),accepted:new Set()},socal:{rejected:new Set(),accepted:new Set()}};
let rejected=sessions.v1.rejected,accepted=sessions.v1.accepted;let current=[],lastInput;
...
function card(a,primary){return '<article class="'+(primary?'primary':'alternative')+'">'+(primary?'<div class="eyebrow">YOUR BEST FIT</div>':'')+'<'+(primary?'h2':'h3')+'>'+escape(a.title)+'</'+(primary?'h2':'h3')+'><div class="meta"><span>'+(a.totalMinutes??a.minutes)+' min</span><span>'+((a.totalCost??a.cost)?'~$'+(a.totalCost??a.cost):'Free*')+'</span><span>'+escape(a.category)+'</span></div><p class="description">'+escape(a.step)+'</p>'+practical(a)+'<div class="why"><strong>Why this fits</strong>'+escape(a.reasons)+'</div><div class="feedback"><button data-action="accept" data-id="'+a.id+'">'+(accepted.has(a.id)?'✓ Enjoy your time':'Sounds good')+'</button><button data-action="reject" data-id="'+a.id+'">Not feeling it</button></div></article>';}
...
function run(){if(main.hidden)return [];lastInput=inputs();current=recommend(lastInput,rejected);render();return current.map(({id,title,reasons})=>({id,title,reasons}));}
...
const titleScreen=document.querySelector('#title-screen'),main=document.querySelector('main'),nav=document.querySelector('#edition-nav');
const welcome=result.innerHTML;
form.day.value=String(new Date().getDay());
document.querySelectorAll('[data-edition]').forEach(button=>button.addEventListener('click',()=>{
 edition=button.dataset.edition;({rejected,accepted}=sessions[edition]);
 document.body.classList.toggle('socal',edition==='socal');
 document.querySelector('#socal-controls').hidden=edition!=='socal';
 document.querySelector('#edition-label').textContent=edition==='socal'?'Sunny SOCAL Expansion!':'Version 1';
 document.querySelector('.checkin h1').innerHTML=edition==='socal'?'A little sunshine.<br>A new direction.':'Find your next<br>little adventure.';
 titleScreen.hidden=true;main.hidden=false;nav.hidden=false;current=[];lastInput=undefined;result.innerHTML=welcome;
 document.querySelector('.checkin h1').tabIndex=-1;document.querySelector('.checkin h1').focus();
}));
document.querySelector('#back-title').addEventListener('click',()=>{
 stopFanfare();document.querySelector('.confetti-layer')?.remove();main.hidden=true;nav.hidden=true;titleScreen.hidden=false;document.body.classList.remove('socal');document.querySelector('#title-heading').focus();
});
function practical(a){
 if(edition!=='socal')return '';
 const source=sources[a.source];
 return '<div class="destination"><p>'+escape(a.neighborhood|| (a.place==='home'?'At home':'Nearby; confirm your route'))+' · '+escape(a.setting)+'</p>'+(source?'<p><a href="'+escape(source.url)+'" target="_blank" rel="noopener noreferrer">Official visitor information ↗</a></p><p>Source checked '+source.verified+'. Suggestion only; hours, tickets, conditions and availability are not confirmed.</p>':'')+'</div>';
}
